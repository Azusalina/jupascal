import { personalGrades } from "./personalDefaults";
import allowedCodes from "../../data/personal/programme_codes.json";
...
export const PROGRAMME_CODE_PATTERN = new RegExp(`^(?:${allowedCodes.join("|")})$`);
...
export function sanitizeGrades(rawGrades: unknown): StudentGrades {
  ...
  for (const [rawSubject, rawGrade] of Object.entries(rawGrades)) {
    if (rawSubject === "m12:module") {
      if (rawGrade === M1_SUBJECT || rawGrade === M2_SUBJECT) grades[rawSubject] = rawGrade;
      continue;
    }
...
function decodeBinary(payload: string): HashState | null {
  try {
    ...
    // Retake controls are unavailable in the personal form.
    if (retakenSubjects) retakenSubjects = [];

    if (Object.keys(grades).length === 0 && pickedCodes.length === 0) return null;
    return { grades: personalGrades(grades), pickedCodes, sharing, showScores, name, mode, ...(retakenSubjects && retakenSubjects.length ? { retakenSubjects } : {}) };
  } catch (e) {
