...
import { hasCompletePersonalGrades } from "../lib/personalDefaults";
...
export function AdvisorConsole({
  ...
  const [gradeCollapsed, setGradeCollapsed] = useState(() => hasCompletePersonalGrades(grades));

  // Profiles may carry a complete saved grade set. Re-evaluate only when the
  // active profile changes so normal editing never causes an unwanted reopen.
  useEffect(() => {
    setGradeCollapsed(hasCompletePersonalGrades(grades));
  }, [activeProfileId]);
      ...
      <aside className="console-rail">
        ...
        <div className="desktop-grade-column">
          <GradeInput
            ...
            onComplete={() => setGradeCollapsed(true)}
          />
        </div>
