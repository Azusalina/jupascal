...
import { localized, MASTER_DATA_SCOPE, masterPathwaysFor, salariesFor } from "../lib/masterPathways";
...
import type { CalculationResult, CandidateScore, EligibilityDetail, HkustFormulaStep, OfferStatistic, Programme, ProgrammeResult } from "../types/jupas";
...
function RetakeDetailNote({ programme, isRetaker, t, lang }: {
  programme: Programme;
  isRetaker: boolean;
  t: Translate;
  lang: Lang;
    ...
    <div className="detail-layout">
      ...
      >
        ...
        <div className="detail-two-column">
          <div className="detail-source-column">
            <a
              className="detail-official-description"
              href={safeHref((programme.programme_websites || []).find(Boolean)) || safeHref(programme.jupas_url) || `https://www.jupas.edu.hk/en/programme/${programme.institution.toLowerCase()}/${programme.jupas_code}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <span>{t("detail.officialDescription")}</span>
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                <path d="M6 3h7v7M13 3 5 11" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M11 9v4H3V5h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </a>
        ...
        <section className={`score-context band-${result.band}`}>
          ...
          <RetakeDetailNote programme={programme} isRetaker={isRetaker} t={t} lang={lang} />
          {(() => {
            ...
            const cards: Array<{ key: string; score: number }> = refKeys
          ...
          {programme.expected_score_2026 != null ? (
            <p className="benchmark-card"><span>{t("detail.expected2026")}</span> <strong>{programme.expected_score_2026}</strong></p>
          ) : null}
          {programme.flexible_admissions_2026 ? <p className="muted">{t("detail.flexible2026")}</p> : null}
          {programme.historical_comparable === false ? <p className="muted benchmark-caveat">{t("detail.historicalOnly")}</p> : null}
        ...
        <OfficialLinksCard programme={programme} />
          ...
          <PersonalValuePanel programme={programme} />
        </div>
      </aside>
    </div>
  );
...
function PersonalValuePanel({ programme }: { programme: Programme }) {
  const { t, lang } = useLang();
  const value = personalValueFor(programme);
  const [masterOpen, setMasterOpen] = useState(false);

  useEffect(() => setMasterOpen(false), [programme.jupas_code]);
  useEffect(() => {
  ...
  return (
    <section className={`personal-value-panel personal-${value.band}`} aria-label={t("personal.title")}>
      <div className="personal-value-hero">
        <div>
          <p className="eyebrow">{t("personal.eyebrow")}</p>
          <h3>{t("personal.title")}</h3>
          <p>{t("personal.independent")}</p>
        </div>
        <div className="personal-value-total">
          <strong>{value.score.toFixed(1)}</strong>
          <span>/ 100</span>
          <em>{t(`personal.band.${value.band}`)}</em>
        </div>
      </div>

      <div className="personal-snapshot">
        <div>
          <span>{t("personal.strongest")}</span>
          <p>{value.strongest.map((row) => row.label[lang]).join(" · ")}</p>
        </div>
        <div>
          <span>{t("personal.gaps")}</span>
          <p>{value.gaps.map((row) => row.label[lang]).join(" · ")}</p>
        </div>
      </div>

      <button
        className="master-pathways-button"
        type="button"
      ...
      >
        <span>
          <b>{t("master.button")}</b>
          <small>{t("master.buttonSub", { n: masterPathwaysFor(programme).length })}</small>
        </span>
      ...
      <section className="personal-formula-card">
        <span>{t("personal.formulaTitle")}</span>
        <code>{t("personal.formula")}</code>
        <p>{t("personal.formulaExplain")}</p>
      </section>

      <div className="personal-parameter-heading">
        <div>
          <span>{t("personal.parameters")}</span>
          <p>{t("personal.parametersExplain")}</p>
        </div>
        <b>{t("personal.weightTotal")}</b>
      </div>

      <div className="personal-parameter-list">
        {value.rows.map((row) => (
          <article className="personal-parameter-row" key={row.key}>
            <div className="personal-parameter-top">
              <div>
                <strong>{row.label[lang]}</strong>
                <p>{row.description[lang]}</p>
              </div>
              <b>{row.contribution.toFixed(1)}<em>/{row.weight}</em></b>
            </div>
            <div className="personal-scale" aria-hidden="true">
              <span style={{ width: `${row.programmeValue * 10}%` }} />
              <i style={{ left: `${row.target * 10}%` }} />
            </div>
            <div className="personal-parameter-meta">
              <span>{t("personal.programmeValue", { value: row.programmeValue })}</span>
              <span>{t("personal.idealValue", { value: row.target })}</span>
              <span>{t("personal.weight", { value: row.weight })}</span>
              <span>{t("personal.match", { value: Math.round(row.match * 100) })}</span>
            </div>
          </article>
        ))}
      </div>

      <p className="personal-caveat">{t("personal.caveat")}</p>

      {masterOpen ? (
        <MasterPathwaysDrawer programme={programme} onClose={() => setMasterOpen(false)} />
      ) : null}
    </section>
  );
}

function MasterPathwaysDrawer({ programme, onClose }: { programme: Programme; onClose: () => void }) {
  const { t, lang } = useLang();
  const routes = masterPathwaysFor(programme);

  return createPortal(
    <>
      <div className="master-drawer-backdrop" role="presentation" onMouseDown={onClose} />
      <section className="master-drawer" role="dialog" aria-modal="true" aria-labelledby="master-drawer-title">
        <header className="master-drawer-header">
          <div>
            <p className="eyebrow">{t("master.eyebrow", { code: programme.jupas_code })}</p>
            <h2 id="master-drawer-title">{t("master.title")}</h2>
            <p>{t("master.subtitle")}</p>
          </div>
          <button className="master-drawer-close" type="button" onClick={onClose} aria-label={t("master.close")}>
        ...
        <div className="master-drawer-scope">
          <strong>{t("master.scopeTitle")}</strong>
          <span>{localized(MASTER_DATA_SCOPE, lang)}</span>
        </div>

        <div className="master-route-list">
          {routes.map((route, index) => {
            const salaries = salariesFor(route);
            return (
              <article className="master-route-card" key={route.id}>
                <div className="master-route-title">
                  <span>{t("master.route", { n: index + 1 })}</span>
                  <div>
                    <h3>{localized(route.name, lang)}</h3>
                    <p>{route.institution} · {route.sourceCycle}</p>
                  </div>
                </div>

                <dl className="master-facts">
                  <div>
                    <dt>{t("master.entry")}</dt>
                    <dd>{localized(route.entryBackground, lang)}</dd>
                  </div>
                  <div>
                    <dt>{t("master.gpa")}</dt>
                    <dd>{localized(route.gpa, lang)}</dd>
                  </div>
                  <div>
                    <dt>{t("master.curriculum")}</dt>
                    <dd>{localized(route.curriculum, lang)}</dd>
                  </div>
                </dl>

                <section className="master-jobs">
                  <h4>{t("master.jobs")}</h4>
                  <div>{route.jobs.map((job) => <span key={job.en}>{localized(job, lang)}</span>)}</div>
                </section>

                <section className="master-salary-section">
                  <h4>{t("master.salary")}</h4>
                  <p className="master-salary-note">{t("master.salaryNote")}</p>
                  <div className="master-salary-list">
                    {salaries.map((salary) => (
                      <article key={salary.id} className="master-salary-card">
                        <div>
                          <strong>{localized(salary.role, lang)}</strong>
                          <b>{salary.monthly}<em>{t("master.perMonth")}</em></b>
                        </div>
                        <p>{localized(salary.basis, lang)}</p>
                        <a href={salary.sourceUrl} target="_blank" rel="noopener noreferrer">{salary.sourceLabel} ↗︎</a>
                      </article>
                    ))}
                  </div>
                </section>

                {route.caution ? <p className="master-caution">{localized(route.caution, lang)}</p> : null}

                <footer className="master-route-source">
                  <span>{t("master.checked", { date: route.checkedOn })}</span>
                  <a href={route.sourceUrl} target="_blank" rel="noopener noreferrer">
                    {t("master.officialSource", { source: route.sourceLabel })} ↗︎
                  </a>
                </footer>
              </article>
            );
        ...
        <footer className="master-drawer-footer">
          <p>{t("master.disclaimer")}</p>
          <button className="ghost-button" type="button" onClick={onClose}>{t("master.close")}</button>
        </footer>
      </section>
    </>,
    document.body,
...
function AuditRows({ candidates, formula, basisYear, t, lang }: { candidates: CandidateScore[]; formula?: string | null; basisYear: "2026"; t: Translate; lang: Lang }) {
