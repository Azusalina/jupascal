import { defaultGrades, hasEnteredGrades } from "./lib/personalDefaults";
...
function CalculatorApp() {
  ...
  const [consoleView, setConsoleView] = useState<"analyze" | "browse" | "detail">("browse");
  ...
  const [browseDisplay, setBrowseDisplay] = useState<"table" | "tree">("tree");
  ...
  const [detailFromAnalysis, setDetailFromAnalysis] = useState(false);
  const [detailFromTree, setDetailFromTree] = useState(false);
  ...
  // Return to the programme list whenever the app loads or refreshes.
  ...
  function openTreeMajorDetail(code: string) {
    if (isDesktop) {
      openBrowseDetail(code);
      return;
    }
    setActiveCode(code);
    setDetailFromAnalysis(false);
    setDetailFromTree(true);
    setAnalysisOpen(false);
    setMobileDetailDirection("forward");
    setMobileDetailOpen(true);
    setStep(3);
  ...
  const filteredResults = useMemo(() => {
    ...
    if (selectedOnly && (filters.eligibleOnly || filters.band !== "all")) {
      return sortResults(filterResults(baseResults, filters), sortKey, sortDirection, deltaMode);
    }
  ...
  function addProfile(rawName: string) {
    ...
    const newProfile: Profile = { id, name, grades: defaultGrades(), pickedCodes: [] };
    setProfiles((prev) => [...prev, newProfile]);
  ...
  const programmePicker = showProgrammeLoading ? (
    ...
    <section className={`panel step2-panel${browseDisplay === "tree" ? " tree-mode" : ""}`} aria-label={t("app.comparisonAria")}>
      {browseDisplay === "tree" ? (
        <Suspense fallback={<div className="pathway-loading" role="status">{t("app.loading.text")}</div>}>
          <PathwayTree
            results={allResults}
            grades={grades}
            onOpenMajor={openTreeMajorDetail}
            onShowList={() => setBrowseDisplay("table")}
          />
        </Suspense>
      ) : (
        <>
          <div className="browse-display-switch" role="group" aria-label={t("tree.displayAria")}>
            <span>{t("tree.displayLabel")}</span>
            <button type="button" className="active" aria-pressed="true" onClick={() => setBrowseDisplay("table")}>{t("tree.displayTable")}</button>
            <button type="button" aria-pressed="false" onClick={() => setBrowseDisplay("tree")}>{t("tree.displayTree")}</button>
          </div>
          <FiltersBar
            filters={filters}
            ...
            onSortChange={(nextSortKey) => {
              ...
              setSortDirection(nextSortKey === "code" || nextSortKey === "name" ? "asc" : "desc");
            }}
  ...
  const mobileDetailNode = (pickedCount > 0 || previewResult)
    ? (
      <div
        key={step3SubView}
    ...
    return (
      <main className="app-shell layout-desktop">
        ...
        <AdvisorConsole
          ...
          onGradesReset={() => { setGrades(defaultGrades()); setRetakenSubjects([]); }}
        ...
        <div className={`stepper-content vt-${stepDirection}`}>
          <div className={step === 1 ? "stepper-panel active" : "stepper-panel"}>
            <GradeInput
              grades={grades}
              onChange={setGrades}
              onReset={() => { setGrades(defaultGrades()); setRetakenSubjects([]); }}
          ...
          <div className={`stepper-footer-right${step === 3 ? " is-share-mode" : ""}`}>
            {step < 3 ? (
              <button
                ...
                disabled={
                  readOnly ||
                  (step === 1 && !hasEnteredGrades(grades)) ||
                  (step === 2 && pickedCount === 0)
                }
                onClick={() => {
                  if (step === 1) setGrades(defaultGrades());
...
function loadProfiles(): Profile[] {
  ...
  return [{ id: "default", name: defaultProfileName(), grades: defaultGrades(), pickedCodes: [] }];
}
...
function sanitizeProfiles(rawProfiles: unknown): Profile[] {
  ...
  return rawProfiles.slice(0, 8).flatMap((profile, index) => {
    ...
    return [{ id, name, grades, pickedCodes: picks }];
  });
}
